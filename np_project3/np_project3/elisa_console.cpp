...
{
public:
	client(boost::asio::io_context &io_context_, int port, std::string addr, std::string file)
    : socket(io_context_),serverPort(port), serverAddr(addr), testFile(file){}
...
void init_clients()
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        if (addr.length() != 0)
        {
            clients_arr.push_back(client(io_context_, stoi(port), addr, "test_case/"+file));
        }
        else
        {
            break;
